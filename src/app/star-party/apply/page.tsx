...
const ApplyPage = () => {
  ...
  const uploadToSupabase = async (file: File, path: string, applicantId: string): Promise<string | null> => {
    try {
      ...
      formData.append('partyType', 'star-party'); // 파티 타입 전달
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm() && !isSubmitting) {
      ...
      try {
        ...
        // 신분증 사진 업로드
        if (formData.idPhoto) {
          const path = `star-party/${id}_id_${formData.idPhoto.name}`;
        ...
        if (formData.personalPhotos.length > 0) {
          const uploadPromises = formData.personalPhotos.map(async (photo, index) => {
            const path = `star-party/${id}_personal_${index + 1}_${photo.name}`;
        ...
        if (formData.jobProofPhotos.length > 0) {
          const uploadPromises = formData.jobProofPhotos.map(async (photo, index) => {
            const path = `star-party/${id}_jobproof_${index + 1}_${photo.name}`;
            return await uploadToSupabase(photo, path, id);
          });
          const urls = await Promise.all(uploadPromises);
            ...
            <div className="bg-white rounded-lg p-4 md:p-6 mt-4 md:mt-8 border border-gray-200">
              ...
              <div className="space-y-3 text-sm text-gray-700">
                <div>
                  ...
                  <br />
                  <span className="text-xs text-gray-600">※ 나이, 이름, 직업만 기입하면 누구나 참석 가능!</span>
                </div>
                <div>
                ...
                <div>
                  <span className="font-semibold text-[#0e6d62]">특징:</span>
                  <br />
                  <span>• 자리도 자유롭게 섞고, 번호 교환도 자유롭게!</span>
                  <br />
                  <span>• 인원 제한 없이 누구나 부담없이 참석 가능</span>
                  <br />
                  <span>• 2차는 원할 때 자유롭게! N분의 1로 부담 없는 즐거움</span>
                </div>
